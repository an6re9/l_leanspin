...
function burgerMenu() {
  ...
  const navMenu = document.querySelector(".burger__navigation-menu");
  const media = 768; // на какой ширине переключаемся на бургер
  const styleAttr = "style";
  const navMenuMobile = "burger__navigation-menu_mobile";
  ...
  function burgerResize() {
    if (window.innerWidth > media) {
      document.body.removeAttribute(styleAttr);
      navMenu.classList.remove(navMenuMobile);
      burger.classList.remove(burgerActive);
      // при ресайзе с большого экранаблок скрываем без анимации
    } else if (!burger.classList.contains(burgerActive)) {
  ...
  function burgerClick(event) {
    ...
    if (burger.classList.contains(burgerActive)) {
      ...
      navMenu.removeAttribute(styleAttr);
...
function slider() {
  ...
  sliders.forEach((slider) => {
    const dataAttr = "[data-link]";
    const links = slider.querySelectorAll(`.slider__links a${dataAttr}`);
    const paginationContainer = slider.querySelector(
      ".slider__pagination-container"
    );
    const paginationItems = slider.querySelectorAll(
      `.slider__pagination-item${dataAttr}`
    );
    const paginationItem = "slider__pagination-item";
    const activeClass = "active";
    const slideVisible = "slider__slides-item_visible";
    const slide = "slider__slides-item";
    ...
    paginationItems[0].classList.add(activeClass);
    ...
    function trigger(dataLink) {
      slider.querySelectorAll(dataAttr).forEach((element) => {
        element.classList.remove(activeClass, slideVisible);
        ...
        if (element.dataset.link === dataLink) {
          if (element.classList.contains(slide)) {
            ...
            if (element.classList.contains(paginationItem))
    ...
    function dragSlider() {
      ...
      const slides = Array.from(
        slider.querySelectorAll(".slider__pagination-item")
      );
      ...
      function stop() {
        ...
        for (let i = 0; i < slides.length; i++) {
          ...
          // если перетащили дальше середины слайда, то перелистываем его
          if (
            offset < slidesWidth &&
      ...
      function resize() {
        ...
        pagintaionResize = setTimeout(() => {
          paginationTrigger(
            slider.querySelector(".slider__pagination-item.active")
          );
        }, 300);
      }
    }
...
function runningString() {
  ...
  document.querySelectorAll(".ticker__container").forEach((strContainer) => {
...
function outlineForItems(e) {
  const blockSelector = "proposal__item";
  const blocks = document.querySelectorAll(`.${blockSelector}`);
  const hoveredSelector = "proposal__item_hovered";

  blocks.forEach((block) => {
    block.addEventListener("mouseover", hoverEffect),
      block.addEventListener("mouseout", hoverEffect);
  });

  function hoverEffect(e) {
    this.classList.toggle(hoveredSelector);
  }
}
...
function hoverLinks() {
  ...
  links.forEach((link) => {
    link.addEventListener("mouseover", mouseHandler),
      link.addEventListener("mouseout", mouseHandler);
    // обработка для мобилок, чтобы hover-эффект не цеплялся к нажатому ранее элементу
    slider.addEventListener("touchmove", hoveredEnd);
  });

  function mouseHandler(e) {
    e.target.classList.toggle("hovered-link");
  }

  function hoveredEnd(e) {
    clearTimeout(interval);
    interval = setInterval(
      () => links.forEach((link) => link.classList.remove("hovered-link")),
      100
    );
  }
}
